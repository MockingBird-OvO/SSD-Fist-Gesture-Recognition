...
![Loss with alpha](_04_training/loss/exp/exp0.png)
...
In industrial projects, `main.py` rarely contains hardcoded numbers.  

**Mindset shift**: Don't mix logic and parameters together.  
**Approach**: Create a `config.py`.  
...
(This project is too small for interruptions)  
...
**Benefit**: You can resume training precisely from any epoch instead of starting over.

#### 3. Monitoring Thinking: Don't Just Look at Loss Numbers
Scrolling numbers in the terminal are hard to read and understand.  
(I only used matplotlib for comparison)  
**Mindset shift**: Humans understand curves and visuals much better.  
**Approach**: Use TensorBoard or WandB.  
**Benefit**: You can monitor loss curves in real-time. You can even visualize model predictions on sample images during training.
...
**Approach**: Separate files, makes it compatible if only needs prediction later.  
...
**Problem**: `Index out of range` error or false matching.
...
**Root cause**: Data loading and augmentation on CPU is too slow (IO bottleneck). GPU is waiting for data.  
